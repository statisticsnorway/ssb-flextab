...
class TestParseFmtSpecValid:
    ...
    def test_documented_examples(self, spec: str, value: float, expected: str) -> None:
    ...
    def test_zero_formats_with_requested_decimals(self) -> None:
        f = _parse_fmt_spec("7,2")
        assert f(0) == "0,00"

    def test_combined_underscore_and_space_modifier_prefers_space(self) -> None:
        f = _parse_fmt_spec("7.1_s")
        assert f(1234.5) == "1 234.5"
        g = _parse_fmt_spec("7,1_s")
        assert g(1234.5) == "1 234,5"

    def test_non_numeric_value_falls_back_to_str(self) -> None:
        f = _parse_fmt_spec("7.1")
        assert f("abc") == "abc"
        assert f(None) == "None"

    def test_leading_zero_width_still_parses(self) -> None:
...
class TestParseFmtSpecInvalid:
    ...
    def test_raises_value_error(self, spec: str) -> None:
...
class TestFormatDataframe:
    def test_default_fmt_applied_to_every_cell(self, plain_df: pd.DataFrame) -> None:
        out = _format_dataframe(plain_df)
        assert out.loc[0, "a"] == "1.235"
        assert out.loc[2, "b"] == "300.000"

    def test_na_rep_used_for_missing_values(self, plain_df: pd.DataFrame) -> None:
        out = _format_dataframe(plain_df, na_rep="MISSING")
        assert out.loc[1, "a"] == "MISSING"

    def test_custom_default_fmt_string(self, plain_df: pd.DataFrame) -> None:
        out = _format_dataframe(plain_df, fmt="{:.0f}")
        assert out.loc[0, "a"] == "1"
        assert out.loc[0, "b"] == "100"

    def test_col_fmt_map_overrides_default_for_that_column(
        self, plain_df: pd.DataFrame
    ) -> None:
        df = plain_df.copy()
    ...
    def test_row_fmt_map_overrides_default_for_that_row(
        self, plain_df: pd.DataFrame
    ) -> None:
        df = plain_df.copy()
    ...
    def test_col_fmt_map_takes_precedence_over_row_fmt_map(
        self, plain_df: pd.DataFrame
    ) -> None:
        df = plain_df.copy()
    ...
    def test_non_numeric_column_falls_back_to_str(self) -> None:
        df = pd.DataFrame({"a": ["x", "y"]})
        out = _format_dataframe(df)
        assert out.loc[0, "a"] == "x"
        assert out.loc[1, "a"] == "y"

    def test_result_columns_are_string_typed_not_numeric(
        self, plain_df: pd.DataFrame
    ) -> None:
        out = _format_dataframe(plain_df)
    ...
    def test_preserves_original_index_and_columns(self, plain_df: pd.DataFrame) -> None:
        out = _format_dataframe(plain_df)
...
class TestFlextabToString:
    def test_basic_rendering_contains_formatted_values(self) -> None:
    ...
    def test_custom_fmt_and_na_rep(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
...
class TestFlextabToMarkdown:
    def test_multiindex_columns_are_flattened_with_separator(self) -> None:
    ...
    def test_suppressed_label_is_dropped_not_left_blank(self) -> None:
    ...
    def test_multiindex_rows_produce_one_column_per_level(self) -> None:
    ...
    def test_unnamed_index_gets_blank_header_cell(self) -> None:
        r = FlextabResult(
    ...
    def test_output_has_correct_number_of_rows(self) -> None:
...
class TestToHex:
    ...
    def test_named_colors_case_insensitive(self) -> None:
        assert FlextabResult._to_hex("BLUE") == "0000FF"

    def test_hex_string_with_and_without_hash(self) -> None:
...
class TestResolveColor:
    ...
    def test_single_color_used_for_every_index(self) -> None:
        assert FlextabResult._resolve_color("blue", 0) == "blue"
        assert FlextabResult._resolve_color("blue", 7) == "blue"

    def test_two_tuple_cycles_by_parity(self) -> None:
        spec = ("red", "blue")
    ...
    def test_three_element_rgb_tuple_is_not_treated_as_cycling_pair(self) -> None:
        spec = (70, 114, 196)
...
class TestFmtToExcelNumfmt:
    ...
    def test_matches_closure_inspected_parameters(
        self, spec: str, expected: str
    ) -> None:
        formatter = _parse_fmt_spec(spec)
        assert FlextabResult._fmt_to_excel_numfmt(formatter) == expected

    def test_arbitrary_callable_without_closure_falls_back_to_sample_parse(
        self,
    ) -> None:
        def plain_fmt(v: float) -> str:
            return f"{v:.2f}"

        assert FlextabResult._fmt_to_excel_numfmt(plain_fmt) == "0.00"

    def test_callable_that_raises_falls_back_to_general(self) -> None:
        def broken_fmt(v: float) -> str:
...
class TestReprAndStr:
    def test_default_repr_uses_default_fmt_of_one_decimal(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
    ...
    def test_custom_default_fmt_attr_is_honored(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
...
class TestReprHtml:
    def test_no_style_set_renders_a_table_with_the_income_header(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
        html = r._repr_html_()
    ...
    def test_default_fmt_is_applied_even_when_no_style_key_is_set(self) -> None:
    ...
    def test_no_style_and_no_default_fmt_still_renders_plain_values(self) -> None:
    ...
    def test_formatting_is_applied_once_any_style_key_is_set(self) -> None:
    ...
    def test_header_bg_excludes_the_row_header_cell(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
    ...
    def test_row_header_bg_colors_only_the_row_header_cell(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
    ...
    def test_cell_bg_cycles_by_data_row(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
...
class TestToExcel:
    def test_col_fmt_map_sets_number_format_on_the_right_column_only(
        self, tmp_path: Path
    ) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult(
    ...
    def test_row_fmt_map_sets_number_format_across_that_data_row(
        self, tmp_path: Path
    ) -> None:
        cols = pd.MultiIndex.from_tuples([("SUM", "Income"), ("MEAN", "Income")])
        idx = pd.Index(["M", "F"], name="Sex")
    ...
    def test_style_colors_header_row_and_cell_fills(self, tmp_path: Path) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
        r.attrs["style"] = {
            "header_bg": "lightgrey",
    ...
    def test_values_remain_numeric_not_stringified(self, tmp_path: Path) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
...
class TestInvalidUsage:
    ...
    def test_unrecognised_color_in_style_dict_raises_when_rendering_html(self) -> None:
        idx = pd.Index(["M", "F"], name="Sex")
        r = FlextabResult({"Income": [100.0, 200.0]}, index=idx)
    ...
    def test_unrecognised_color_in_style_dict_raises_on_to_excel(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_empty_dataframe_formats_without_error(self) -> None:
        df = pd.DataFrame({"a": pd.Series([], dtype=float)})
        out = _format_dataframe(df)
        assert len(out) == 0

    def test_markdown_on_single_row_single_column(self) -> None:
