...
class TestWeightCleaning:
    def test_negative_weights_zeroed_positive_untouched(self) -> None:
    ...
    def test_drop_nan_x_keeps_x_and_w_aligned(self) -> None:
    ...
    def test_drop_nan_x_on_all_valid_is_a_no_op(self) -> None:
        x = pd.Series([1.0, 2.0])
        w = pd.Series([5.0, 6.0])
        xx, ww = _drop_nan_x(x, w)
...
class TestWeightedMean:
    def test_equal_weights_matches_plain_mean(self) -> None:
        x = pd.Series([1.0, 2.0, 3.0])
        w = pd.Series([1.0, 1.0, 1.0])
        assert _wmean(x, w) == approx(x.mean())

    def test_unequal_weights_matches_hand_computation(self) -> None:
        x = pd.Series([1.0, 2.0, 3.0])
        w = pd.Series([1.0, 2.0, 3.0])
        assert _wmean(x, w) == approx((1 * 1 + 2 * 2 + 3 * 3) / (1 + 2 + 3))

    def test_zero_total_weight_returns_nan(self) -> None:
        x = pd.Series([1.0, 2.0])
        w = pd.Series([0.0, 0.0])
        assert math.isnan(_wmean(x, w))

    def test_nan_measure_row_excluded_from_both_numerator_and_denominator(self) -> None:
        x = pd.Series([1.0, np.nan, 3.0])
        w = pd.Series([10.0, 20.0, 30.0])
...
class TestWeightedVariance:
    def test_all_weights_equal_one_matches_sample_variance(self) -> None:
        x = pd.Series([1.0, 2.0, 3.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0, 1.0])
        assert _wvar(x, w) == approx(x.var())  # pandas default ddof=1

    def test_single_observation_is_degenerate_nan(self) -> None:
        x = pd.Series([5.0])
        w = pd.Series([2.0])
    ...
    def test_std_is_sqrt_of_var(self) -> None:
        x = pd.Series([1.0, 2.0, 3.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0, 1.0])
        assert _wstd(x, w) == approx(math.sqrt(_wvar(x, w)))

    def test_std_propagates_nan_from_degenerate_variance(self) -> None:
        x = pd.Series([5.0])
        w = pd.Series([2.0])
        assert math.isnan(_wstd(x, w))

    def test_stderr_is_std_over_sqrt_wsum(self) -> None:
        x = pd.Series([1.0, 2.0, 3.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0, 1.0])
        expected = _wstd(x, w) / math.sqrt(w.sum())
        assert _wstderr(x, w) == approx(expected)

    def test_stderr_nan_when_wsum_zero(self) -> None:
        x = pd.Series([1.0, 2.0])
        w = pd.Series([0.0, 0.0])
...
class TestWeightedPercentile:
    def test_equal_weights_median(self) -> None:
    ...
    def test_heavier_weight_pulls_percentile_toward_that_value(self) -> None:
    ...
    def test_all_nan_measure_returns_nan(self) -> None:
...
class TestWeightedGeometricAndHarmonicMean:
    def test_wgmean_equal_weights_matches_unweighted_gmean(self) -> None:
        x = pd.Series([1.0, 2.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0])
        expected = math.exp((math.log(1) + math.log(2) + math.log(4)) / 3)
        assert _wgmean(x, w) == approx(expected)

    def test_wgmean_excludes_non_positive_values(self) -> None:
        x = pd.Series([-1.0, 0.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0])
        assert _wgmean(x, w) == approx(4.0)

    def test_wgmean_all_non_positive_returns_nan(self) -> None:
        x = pd.Series([-1.0, 0.0])
        w = pd.Series([1.0, 1.0])
        assert math.isnan(_wgmean(x, w))

    def test_whmean_equal_weights_matches_unweighted_hmean(self) -> None:
        x = pd.Series([1.0, 2.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0])
        assert _whmean(x, w) == approx(_hmean(x))

    def test_whmean_excludes_zero_values(self) -> None:
        x = pd.Series([0.0, 2.0, 4.0])
        w = pd.Series([1.0, 1.0, 1.0])
        expected = (1.0 + 1.0) / (1.0 / 2.0 + 1.0 / 4.0)
        assert _whmean(x, w) == approx(expected)

    def test_whmean_all_zero_returns_nan(self) -> None:
        x = pd.Series([0.0, 0.0])
        w = pd.Series([1.0, 1.0])
...
class TestParseDenomDef:
    def test_single_token_uppercased(self) -> None:
        assert _parse_denom_def("income") == ["INCOME"]

    def test_multiple_tokens_preserve_order(self) -> None:
        assert _parse_denom_def("gender all") == ["GENDER", "ALL"]

    def test_extra_whitespace_is_collapsed(self) -> None:
...
class TestStatTables:
    def test_all_stats_is_union_of_base_and_percent(self) -> None:
        assert ALL_STATS == set(_BASE_STATS) | _PERCENT_STATS

    def test_every_base_stat_has_a_weighted_counterpart(self) -> None:
    ...
    def test_count_stats_are_never_in_weighted_formula_dict_by_accident(self) -> None:
...
class TestComputeSeriesBasic:
    def test_mean_by_single_group(self, df: pd.DataFrame) -> None:
        s = _compute_series(df, ["sex"], "income", "MEAN", ["sex"], [], missing=False)
        assert s["M"] == approx(15.0)  # (10+20)/2
        assert s["F"] == approx(40.0)  # (30+40+50)/3

    def test_bare_n_needs_no_measure(self, df: pd.DataFrame) -> None:
        s = _compute_series(df, ["sex"], None, "N", ["sex"], [], missing=False)
        assert s["M"] == 2
        assert s["F"] == 3

    def test_sum_by_cross_of_two_groups(self, df: pd.DataFrame) -> None:
        s = _compute_series(
    ...
    def test_grand_total_when_no_groups(self, df: pd.DataFrame) -> None:
        s = _compute_series(df, [], "income", "SUM", [], [], missing=False)
        assert s["__total__"] == 150.0

    def test_missing_true_keeps_nan_group_missing_false_drops_it(self) -> None:
        d = pd.DataFrame(
...
class TestComputeSeriesPercent:
    def test_pctn_sums_to_100_across_full_cross(self, df: pd.DataFrame) -> None:
        s = _compute_series(
            df, ["sex", "region"], None, "PCTN", ["sex"], ["region"], missing=False
        )
        assert s.sum() == approx(100.0)

    def test_pctn_cell_values(self, df: pd.DataFrame) -> None:
    ...
    def test_rowpctn_sums_to_100_within_each_row(self, df: pd.DataFrame) -> None:
        s = _compute_series(
    ...
    def test_colpctn_sums_to_100_within_each_column(self, df: pd.DataFrame) -> None:
        s = _compute_series(
    ...
    def test_rowpctn_without_row_groups_falls_back_to_grand_total(
...
class TestComputeSeriesWeighted:
    def test_weighted_mean_matches_hand_computation(self) -> None:
        d = pd.DataFrame(
            {
                "sex": ["M", "M", "F", "F"],
    ...
    def test_negative_weight_zeroed_but_row_still_counted_in_n(self) -> None:
        d = pd.DataFrame(
            {
                "sex": ["F", "F"],
                "income": [30.0, 40.0],
    ...
    def test_missing_weight_row_excluded_even_from_n(self) -> None:
        d = pd.DataFrame(
            {
                "sex": ["M", "M"],
                "income": [10.0, 20.0],
...
class TestComputeAllSeries:
    def test_all_on_rows_sums_by_remaining_column_group(self, df: pd.DataFrame) -> None:
    ...
    def test_all_on_rows_colpctn_is_always_100(self, df: pd.DataFrame) -> None:
    ...
    def test_all_on_cols_rowpctn_is_always_100(self, df: pd.DataFrame) -> None:
        s = _compute_all_series(
    ...
    def test_all_on_both_dims_collapses_to_grand_total(self, df: pd.DataFrame) -> None:
        s = _compute_all_series(
...
class TestComputeCustomPct:
    def test_denom_token_matching_innermost_groupby_collapses_it(
        self, df: pd.DataFrame
    ) -> None:
    ...
    def test_denom_matching_a_measure_name_uses_full_grouping_ratio(
        self, df: pd.DataFrame
    ) -> None:
    ...
    def test_all_token_fallback_gives_grand_total_denominator(
        self, df: pd.DataFrame
    ) -> None:
...
class TestInvalidUsage:
    def test_mean_without_measure_raises(self, df: pd.DataFrame) -> None:
        with pytest.raises(ValueError, match="requires a measure variable"):
            _compute_series(df, ["sex"], None, "MEAN", ["sex"], [], missing=False)

    def test_nmiss_without_measure_raises_with_specific_guidance(
        self, df: pd.DataFrame
    ) -> None:
        with pytest.raises(ValueError, match="NMISS"):
            _compute_series(df, ["sex"], None, "NMISS", ["sex"], [], missing=False)

    def test_pctsum_without_measure_raises(self, df: pd.DataFrame) -> None:
        with pytest.raises(ValueError, match="requires a measure variable"):
            _compute_series(df, ["sex"], None, "PCTSUM", ["sex"], [], missing=False)

    def test_unknown_statistic_raises(self, df: pd.DataFrame) -> None:
        with pytest.raises(ValueError, match="Unknown statistic"):
            _compute_series(
                df, ["sex"], "income", "NOTASTAT", ["sex"], [], missing=False
            )

    def test_all_series_mean_without_measure_raises(self, df: pd.DataFrame) -> None:
        with pytest.raises(ValueError, match="requires a measure variable"):
            _compute_all_series(
                df,
                ["region"],
    ...
    def test_n_count_size_do_not_require_a_measure(self, df: pd.DataFrame) -> None:
    ...
    def test_zero_denominator_in_row_pct_yields_nan_not_error(self) -> None:
    ...
    def test_empty_dataframe_grand_total_is_nan_or_zero_not_an_exception(self) -> None:
        d = pd.DataFrame(
