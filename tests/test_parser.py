...
class TestSimpleCross:
    ...
    def test_tree_shape_is_cross_of_two_vars(self):
    ...
    def test_expands_to_single_leaf_path(self):
        (root,) = parse_table("income*mean")
        path = only_path(root)
        assert leaf_names(path) == ["income", "mean"]

    def test_classification_recognizes_measure_and_stat(self):
        (root,) = parse_table("income*mean")
        path = only_path(root)
    ...
    def test_repr_round_trips_readably(self):
        (root,) = parse_table("income*mean")
...
class TestTwoDimensionsWithComma:
    ...
    def test_stat_name_case_is_preserved_on_node_but_normalized_on_classify(self):
    ...
    def test_row_classifies_as_a_bare_groupby(self):
...
class TestGroupCrossedWithMeasure:
    ...
    def test_distributes_into_two_leaf_paths(self):
        (root,) = parse_table("(sex region)*income")
    ...
    def test_branch_index_is_zero_when_root_is_not_a_top_level_concat(self):
    ...
    def test_classification_of_each_branch(self):
        (root,) = parse_table("(sex region)*income")
        paths = _expand_node(root)
        specs = [
...
class TestFormatSuffixOnLeaf:
    ...
    def test_format_spec_is_recorded_on_the_measure_node(self):
        (root,) = parse_table("(sex region)*income*format=9,1s")
        paths = _expand_node(root)
        for path in paths:
    ...
    def test_classification_surfaces_the_format(self):
        (root,) = parse_table("(sex region)*income*format=9,1s")
    ...
    def test_format_on_a_group_propagates_to_every_leaf(self):
        (root,) = parse_table("(mean gmean)*format=7,1")
        paths = _expand_node(root)
        assert {n.fmt for p in paths for n in p if n.kind == "var"} == {"7,1"}

    def test_explicit_leaf_format_wins_over_outer_group_format(self):
...
class TestDenominatorAndSuppressedLabel:
    ...
    def test_correct_order_parses_with_denom_and_suppressed_label(self):
    ...
    def test_denom_without_label_suppression_also_works(self):
        (root,) = parse_table("pctsum<region>")
        path = only_path(root)
        assert path[0].denom == "region"
        assert path[0].label is None

    def test_classification_surfaces_denom_def(self):
        (root,) = parse_table("(sex region)*pctsum=''<region>*income*format=9,1s")
        for path in _expand_node(root):
            spec = _classify_path(
                path, measure_list=["income"], groupby_list=["sex", "region"]
...
class TestAllKeywordAndLabels:
    ...
    def test_all_with_double_quoted_label(self):
        (root,) = parse_table('sex ALL="Grand Total"')
        all_node = root.children[1]
        assert all_node.label == "Grand Total"

    def test_all_with_single_quoted_label(self):
        (root,) = parse_table("sex ALL='Grand Total'")
        all_node = root.children[1]
        assert all_node.label == "Grand Total"

    def test_top_level_concat_preserves_branch_order(self):
        (root,) = parse_table("sex ALL='Subtotal'")
        branches = [b for b, _p in _expand_node_with_branch(root)]
        assert branches == [0, 1]

    def test_renamed_groupby_label_is_tracked_separately_from_original_name(self):
...
class TestSplitDimensions:
    ...
    def test_does_not_split_decimal_comma_in_format_spec(self):
        assert _split_dimensions("sex, income*format=9,1s") == [
            "sex",
            " income*format=9,1s",
        ]

    def test_does_not_split_comma_inside_quoted_label(self):
...
class TestInvalidExpressions:
    ...
    def test_more_than_two_dimensions_rejected(self):
    ...
    def test_unopened_paren_raises_syntax_error(self):
        with pytest.raises(SyntaxError):
            parse_table("sex)")

    def test_double_star_raises_syntax_error(self):
        with pytest.raises(SyntaxError):
            parse_table("sex**income")

    def test_trailing_star_raises(self):
        with pytest.raises((SyntaxError, IndexError)):
            parse_table("sex*")

    def test_malformed_format_spec_raises_syntax_error(self):
    ...
    def test_abbreviated_f_equals_is_not_recognized_as_format(self):
    ...
    def test_leading_digit_identifier_raises_syntax_error(self):
        with pytest.raises(SyntaxError):
            parse_table("1income*mean")

    def test_unterminated_quoted_label_raises(self):
        with pytest.raises(SyntaxError):
            parse_table("sex='Gender")

    def test_stray_character_raises_syntax_error(self):
        with pytest.raises(SyntaxError):
            parse_table("income # mean")

    def test_unknown_token_passes_parsing_but_fails_classification(self):
    ...
    def test_various_malformed_expressions_raise(self, expr):
        with pytest.raises((SyntaxError, IndexError)):
            parse_table(expr)

    def test_empty_dimension_between_commas_raises(self):
...
class TestTokenize:
    ...
    def test_labeled_name_token(self):
